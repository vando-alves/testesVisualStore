...
pesquisarjs_versao = '2.00.01';
 ...
 function objpesquisar(nome) {
    ...
    this.executar = function(consulta, filtro, groupby, qtdLinhas) {
        ...
        lefto = (screen.width - 800) / 2;
        if (filtro){
        	if (groupby){
        		if(qtdLinhas){
        			parent.showframe('br/com/visualmix/generico/jsppesquisar/Event.jsp?evento=executar::' + consulta + '|' + filtro + '|' + groupby + '|' + qtdLinhas + '&objeto=' + this.name + '&titulo=' + this.titulo, 580, 435);
        		}else{
        			parent.showframe('br/com/visualmix/generico/jsppesquisar/Event.jsp?evento=executar::' + consulta + '|' + filtro + '|' + groupby + '&objeto=' + this.name + '&titulo=' + this.titulo, 580, 435);
        		}
        	}else{
        		if(qtdLinhas){
        			parent.showframe('br/com/visualmix/generico/jsppesquisar/Event.jsp?evento=executar::' + consulta + '|' + filtro + '|' + '|' + qtdLinhas + '&objeto=' + this.name + '&titulo=' + this.titulo, 580, 435);
        		}else{
        			parent.showframe('br/com/visualmix/generico/jsppesquisar/Event.jsp?evento=executar::' + consulta + '|' + filtro + '&objeto=' + this.name + '&titulo=' + this.titulo, 580, 435);
        		}
        	}
        }else{
        	if(qtdLinhas){
        		parent.showframe('br/com/visualmix/generico/jsppesquisar/Event.jsp?evento=executar::' + consulta + '|||' + qtdLinhas + '&objeto=' + this.name + '&titulo=' + this.titulo, 580, 435);
        	}else{
    ...
    this.executarfiltro = function(consulta, filtro) {
       ...
       this.janela = window.open('br/com/visualmix/generico/jsppesquisar/Event.jsp?evento=executar::' + consulta + '|' + filtro + '&objeto=' + this.name + '&titulo=' + this.titulo,'Window'+ this.name,'scrollbars=yes,width=580,height=435,top=' + topo + ',left='+ lefto);
    }

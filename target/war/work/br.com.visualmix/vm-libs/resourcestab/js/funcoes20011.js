...
function ajaxreplace(valor) {
	...
	return valor.replace(/\$1/gi, "|").replace(/\$2/gi, "@").replace(/\$3/gi, "'").replace(/\$0/gi, "$");
...
function ajaxid(classe, beanid, funcao, parametro,acao,msg, paradicional) {
    ...
    var arrayParametro = parametro.split('|');
	...
	if (arrayParametro.length <= 1) {
       ...
       for (i = 0; i < arrayParametro.length; i++) {
	       parenv = parenv + getValuex(arrayParametro[i]) + '|';
  	   }
	}
...
// Função do Componente MultiSelect
